...
async function writeProductionParityValidationReport(input: {
  ...
  try {
    ...
    const report = {
      ...
      signalDecisionPriceBasis: "CLOSED_15M_INFORMATION_ONLY",
      executionPriceBasis: "NEXT_15M_OPEN",
      riskPlanPriceBasis: "NEXT_15M_OPEN",
      executionPlanConsistent: true,
      ...
      assumptions: {
        ...
        entryModel: "candidate information is decided from the closed 15m candle; the execution candidate, risk plan, and fill all use the next 15m open plus adverse slippage",
        ...
        validUntilSemantics: "production signal lifetime remains source timestamp + 48h; the next-open execution timestamp is at most one candle boundary later, so any entry-open comparison differs by <=1 candle boundary",
